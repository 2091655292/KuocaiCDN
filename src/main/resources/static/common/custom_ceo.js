...
function showDomainCreateProgress(message) {
    ...
    if (messageElement) {
        messageElement.textContent = message || '正在向加速平台提交配置，通常需要几十秒，请耐心等待。';
    }
...
function showDomainVerifyModal(domainName, serviceArea, info, pendingCreatePayload) {
    ...
    verifyRecords.forEach(function(record) {
        const row = $('<tr></tr>')
        if (window.kuocaiLoginRole === 'admin') {
            $('<td></td>').text(record['targetName'] || '默认加速平台').appendTo(row)
        }
...
async function submitDomainCreate(payload, verified = false) {
    ...
    showDomainCreateProgress(verified
        ? '验证已通过，正在向加速平台创建域名并保存到本站，请耐心等待…'
        : '正在向加速平台创建域名并保存到本站，通常需要几十秒，请耐心等待…');
    let data;
    try {
...
async function retryDomainConfiguration(id, button) {
    if (!id) {
        layerWarn("加速域名ID不能为空");
        return;
    }
    ...
    try {
        const data = await sendRequest("POST", "CdnDomain/retryDomainConfig", {id: id});
        autoLayer(data);
        if (data['code'] === 'SUCCESS') {
            setTimeout(function() {
                reload();
    ...
    if (backFlag === '1') {
        ...
        if (window.currentDomainRoute === 'kingsoft') {
            ...
            if (backHttpPort !== finalHttpPort || backHttpsPort !== finalHttpsPort || hostNameBack !== hostNameMain) {
                layerWarn("提醒：当前加速平台限制主备源站使用统一的端口和回源Host配置（以主源站为准）", 5000);
            }
        }
    }
