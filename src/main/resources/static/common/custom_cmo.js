...
async function saveWebsiteBaseConfig() {
    ...
    try {
        ...
        if (!appendWebsiteIconFormData(formdata)) {
            return;
        }
...
function appendWebsiteIconFormData(formdata) {
    const fileInput = document.getElementById("updateWebsiteIconFile");
    const file = fileInput && fileInput.files ? fileInput.files[0] : null;
    if (file) {
        formdata.append("websiteIconImgUrl", "false");
        formdata.append("websiteIconImg", file);
        return true;
    }

    const iconUrl = ($('#websiteIconUrl').val() || '').trim();
    if (iconUrl && !isValidWebsiteImageUrl(iconUrl)) {
        layerWarn("网站图标链接必须使用 http、https 或站内绝对路径");
        return false;
    }
    formdata.append("websiteIconImgUrl", iconUrl || "false");
    return true;
}
...
function appendWebsiteLogoFormData(formdata) {
    ...
    if (logoUrl && !isValidWebsiteImageUrl(logoUrl)) {
...
function isValidWebsiteImageUrl(value) {
...
function isValidWebsiteLogoUrl(value) {
    return isValidWebsiteImageUrl(value);
}

function handleWebsiteIconFileChange() {
    const input = document.getElementById('updateWebsiteIconFile');
    if (input && input.files && input.files.length > 0) {
        $('#websiteIconUrl').val('');
    }
}

function handleWebsiteIconUrlInput() {
    const iconUrl = ($('#websiteIconUrl').val() || '').trim();
    const fileInput = document.getElementById('updateWebsiteIconFile');
    if (iconUrl && fileInput) {
        fileInput.value = '';
    }
}

function previewWebsiteIconUrl() {
    const iconUrl = ($('#websiteIconUrl').val() || '').trim();
    if (!isValidWebsiteImageUrl(iconUrl)) {
        layerWarn("请输入正确的网站图标链接");
        return;
    }
    const preview = new Image();
    preview.onload = function () {
        $('#websiteIconImg').attr('src', iconUrl);
        const fileInput = document.getElementById('updateWebsiteIconFile');
        if (fileInput) {
            fileInput.value = '';
        }
    ...
    preview.onerror = function () {
        layerWarn("网站图标链接无法加载");
    };
    preview.src = iconUrl;
}

function clearWebsiteIcon() {
    $('#websiteIconUrl').val('');
    $('#websiteIconImg').removeAttr('src');
    const fileInput = document.getElementById('updateWebsiteIconFile');
    if (fileInput) {
        fileInput.value = '';
    }
...
function previewWebsiteLogoUrl() {
    const logoUrl = ($('#websiteLogoUrl').val() || '').trim();
    if (!isValidWebsiteImageUrl(logoUrl)) {
...
function appendWebsiteImageFormData(formdata, urlField, fileField, imageSelector, fileSelector) {
    ...
    const persistedSrc = image.attr("data-persisted-src");
    ...
    if (persistedSrc !== undefined) {
        formdata.append(urlField, persistedSrc || "false");
        return;
    }
...
function openWebsiteQrCodeFileChooser(inputId) {
    const input = document.getElementById(inputId);
    if (!input) {
        layerFail("上传控件加载失败，请刷新页面后重试");
        return;
    }
    input.click();
}

function handleWebsiteQrCodeFileChange(input, previewId, statusId) {
    ...
    if (!allowedExtensions.includes(extension)
        ...
        input.value = '';
        layerWarn("二维码仅支持 PNG、JPG、JPEG 或 WebP 图片");
        return;
    }
    if (file.size > 10 * 1024 * 1024) {
        input.value = '';
        layerWarn("二维码图片不能超过 10MB");
        return;
    }
    ...
    reader.onload = function (event) {
        ...
        if (status) {
            ...
            status.textContent = '已选择 ' + file.name + '，点击页面底部“保存网站设置”后生效';
        }
    };
    reader.onerror = function () {
        input.value = '';
        layerFail("图片读取失败，请重新选择");
    };
