...
public class SelfHostedAgentController {
    ...
    private final SelfHostedDomainStatisticsStorageService statisticsStorageService;

    public SelfHostedAgentController(SelfHostedCdnService selfHostedCdnService,
                                     SelfHostedDomainStatisticsStorageService statisticsStorageService) {
        this.selfHostedCdnService = selfHostedCdnService;
        this.statisticsStorageService = statisticsStorageService;
    }

    @PostMapping("statistics")
    public RespResult statistics(@RequestHeader("X-Kuocai-Node-Id") Long nodeId,
                                 @RequestHeader("Authorization") String authorization,
                                 @RequestBody SelfHostedStatisticsBatchRequest request) {
        try {
            SelfHostedNode node = selfHostedCdnService.authenticate(nodeId, authorization);
            boolean stored = statisticsStorageService.ingest(node.getId(), request);
            return RespResult.success(stored ? "ok" : "duplicate");
