...
public class SysConfigController extends BaseController {
    ...
    public RespResult saveWebsiteBaseConfig(String websiteName, String websiteAnnouncement, Double defaultFlowPrice, Integer maxDomainCount,
        ...
        try {
            ...
            websiteBaseConfigVo = WebsiteBaseConfigVo.builder().websiteName(websiteName).websiteAnnouncement(websiteAnnouncement).maxDomainCountProxy(0)
                    ...
                    .websiteIconImg(resolveWebsiteIcon(websiteIconImgUrl, websiteIconImg))
    ...
    private String resolveWebsiteIcon(String iconUrl, MultipartFile iconFile) throws Exception {
        if (!Assert.isEmpty(iconFile)) {
            return ossClient.upload(iconFile);
        }
        if ("false".equals(iconUrl) || Assert.isEmpty(iconUrl)) {
            return "";
        }
        return validateWebsiteIconUrl(iconUrl);
    }

    static String validateWebsiteIconUrl(String iconUrl) throws BusinessException {
        return validateWebsiteImageUrl(iconUrl, "网站图标");
    }

    static String validateWebsiteLogoUrl(String logoUrl) throws BusinessException {
        return validateWebsiteImageUrl(logoUrl, "Logo");
    }

    private static String validateWebsiteImageUrl(String imageUrl, String fieldName) throws BusinessException {
        String value = imageUrl == null ? "" : imageUrl.trim();
        if (Assert.isEmpty(value)) {
            return "";
        }
        if (value.matches(".*[\\s<>\"'].*")) {
            throw new BusinessException(fieldName + "链接格式不正确");
        }
        if (value.startsWith("/") && !value.startsWith("//")) {
            if (value.contains("..") || value.contains("\\")) {
                throw new BusinessException(fieldName + "站内路径格式不正确");
            }
            return value;
        }
        try {
        ...
        throw new BusinessException(fieldName + "链接必须使用 http、https 或站内绝对路径");
    }
