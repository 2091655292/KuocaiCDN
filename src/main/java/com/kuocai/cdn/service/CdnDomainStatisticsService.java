...
public class CdnDomainStatisticsService {
    ...
    public JSONObject mergeAllPlatForm(List<CdnDomain> cdnDomains, DateTime start, DateTime end, String type, Long userId) throws BusinessException {
        ...
        boolean hasSkippedStatisticsFailure = false;
            ...
            for (List<CdnDomain> groupToQuery : processingGroups) {
                ...
                for (List<String> items : splitDistinctCdnDomainList) {
                    ...
                    try {
                        ...
                        log.warn("跳过统计失败批次：route={}, domains={}, type={}, userId={}, msg={}",
                                route, domainNames, type, userId, msg);
                        hasSkippedStatisticsFailure = true;
                    }
                }
            }
        }
        ...
        if (!hasSkippedStatisticsFailure) {
    ...
    static boolean shouldIncludeStatisticsDomain(CdnDomain domain, String route) {
        return domain != null
                ...
                && isStatisticsEligibleStatus(domain.getDomainStatus());
    }

    private static boolean isStatisticsEligibleStatus(String status) {
        return Assert.isEmpty(status)
                || (!"configuring".equals(status)
                && !"configure_failed".equals(status)
