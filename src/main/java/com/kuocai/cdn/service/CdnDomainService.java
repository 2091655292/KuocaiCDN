...
public class CdnDomainService extends BaseService<CdnDomain> implements VoData<CdnDomain, CdnDomainVo> {
    ...
    public static final String USER_DOMAIN_CONFIGURATION_FAILURE =
            "域名配置失败，请检查域名、源站和 DNS 配置；如仍失败请联系管理员。";

    private static final List<String> USER_HIDDEN_PROVIDER_FIELDS = Arrays.asList(
            "route", "primaryRoute", "vendorAccountId", "vendorAccountName", "multiCdn",
            "edgeOneIncluded", "cdnSupplier", "tencentCname", "tencentDnsId", "domainId");
    ...
    public JSONObject queryForDatatables(Long userId, DataTableQuery query) {
        ...
        List<CdnDomainVo> rows = convert2Vo(jsonArray.toJavaList(CdnDomain.class));
        if (Assert.isEmpty(userId)) {
            jsonObject.put("data", rows);
        } else {
            jsonObject.put("data", rows.stream()
                    .map(CdnDomainService::toUserVisibleDomainRow)
                    .collect(Collectors.toList()));
        }
        return jsonObject;
    }

    static JSONObject toUserVisibleDomainRow(CdnDomainVo source) {
        if (source == null) {
            return new JSONObject();
        }
        JSONObject row = (JSONObject) JSONObject.toJSON(source);
        // FastJSON does not honor Jackson's ToStringSerializer annotation. Keep Snowflake IDs as
        // strings so browsers never round them beyond JavaScript's safe integer range.
        if (source.getId() != null) {
            row.put("id", String.valueOf(source.getId()));
        }
