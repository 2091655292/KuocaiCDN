...
public class SelfHostedPortForwardService {
    ...
    public List<JSONObject> list(Long userId, boolean admin) {
        ...
        for (SelfHostedPortForward rule : portForwardDao.selectList(query)) {
            ...
            appendDeploymentStatus(item, rule.getNodeGroupId());
            result.add(item);
        }
        return result;
    }
    ...
    public SelfHostedPortForward save(SelfHostedPortForwardSaveRequest request,
        ...
        if ("enabled".equals(status)) {
            ...
            ensurePortForwardReady(groupId);
        }
        Date now = new Date();
        if (existing == null) {
        ...
        existing.setProxyProtocolEnabled(Boolean.TRUE.equals(request.getProxyProtocolEnabled()) ? 1 : 0);
    ...
    public void setStatus(Long id, boolean enabled, Long loginUserId, boolean admin) throws BusinessException {
        ...
        if (enabled) {
            ...
            ensurePortForwardReady(rule.getNodeGroupId());
        }
    ...
    private void ensurePortForwardReady(Long groupId) throws BusinessException {
        List<String> unavailableNodes = new ArrayList<>();
        for (SelfHostedGroupNode relation : groupNodeDao.selectList(
                new QueryWrapper<SelfHostedGroupNode>().eq("group_id", groupId))) {
            SelfHostedNode node = nodeDao.selectById(relation.getNodeId());
            if (node == null || (node.getEnabled() != null && node.getEnabled() == 0)) {
                continue;
            }
            String warning = SelfHostedCdnService.portForwardCompatibilityWarning(node);
            if (!Assert.isEmpty(warning)) {
                unavailableNodes.add(Assert.isEmpty(node.getNodeName())
                        ? String.valueOf(node.getHost()) : node.getNodeName());
            }
        }
        if (!unavailableNodes.isEmpty()) {
            String names = String.join("、", unavailableNodes.subList(0, Math.min(3, unavailableNodes.size())));
            throw new BusinessException("节点组中有 " + unavailableNodes.size()
                    + " 台节点暂不支持端口转发，请在节点管理中重新填写 SSH 密码并安装 Agent：" + names);
        }
    }

    private void appendDeploymentStatus(JSONObject item, Long groupId) {
        int totalNodes = 0;
        int readyNodes = 0;
        for (SelfHostedGroupNode relation : groupNodeDao.selectList(
                new QueryWrapper<SelfHostedGroupNode>().eq("group_id", groupId))) {
            ...
            totalNodes++;
            if (Assert.isEmpty(SelfHostedCdnService.portForwardCompatibilityWarning(node))) {
                readyNodes++;
            }
        }
        item.put("deploymentTotalNodes", totalNodes);
        item.put("deploymentReadyNodes", readyNodes);
        item.put("deploymentStatus", readyNodes == totalNodes && totalNodes > 0
                ? "ready" : (readyNodes > 0 ? "partial" : "unavailable"));
    }
