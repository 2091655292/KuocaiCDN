...
public class SelfHostedDomainServiceImpl extends BaseService<CdnDomain> implements ICdnPlatformService {
    ...
    public CdnDomain create(Long userId, String domainName, String businessType, String serviceArea,
        ...
        return createForRoute(targetRoute, userId, domainName, businessType, serviceArea, originType,
                ipOrDomain, originProtocol, httpPort, httpsPort, originHost, originWeight);
    }

    public CdnDomain createForRoute(String targetRoute, Long userId, String domainName, String businessType,
        ...
        if (fixedServiceArea != null) {
            if (!fixedServiceArea.equals(serviceArea)) {
                throw new BusinessException("自建 CDN 线路与加速区域不匹配");
            }
    ...
    public CdnDomain configDNS(CdnDomain cdnDomain) throws TencentCloudSDKException, BusinessException {
        try {
            String upstreamCname = prepareForMultiCdn(cdnDomain);
            if (cdnDomain.getTencentDnsId() == null || Assert.isEmpty(cdnDomain.getCname())) {
                CreateRecordDTO dto = buildCustomerCnameRecord(cdnDomain.getDomainName(),
                        TencentDns.LOCAL_DOMAIN_NAME, upstreamCname);
            ...
            CdnDomain saved = save(cdnDomain);
            if (selfHostedCdnService.pauseIfAliyunIcpBlocked(saved)) {
                return saved;
            }
            return saved;
    ...
    public String prepareForMultiCdn(CdnDomain cdnDomain) throws BusinessException {
    ...
    public void enable(CdnDomain cdnDomain) throws BusinessException {
        if (selfHostedCdnService.pauseIfAliyunIcpBlocked(cdnDomain)) {
            throw new BusinessException(AliyunIcpComplianceProbe.USER_MESSAGE);
        }
        selfHostedCdnService.clearAliyunIcpBlock(cdnDomain);
        cdnDomain.setDomainStatus("online");
        cdnDomain.setUpdateTime(new Date());
        save(cdnDomain);
        SelfHostedDomainConfig config = selfHostedCdnService.getDomainConfig(cdnDomain.getId());
    ...
    @Override public void saveCacheRules(CdnDomain d, SettingCacheVo v) throws BusinessException {
        ...
        if (v != null && v.getCacheRules() != null) {
            v.getCacheRules().stream()
                    .filter(rule -> rule != null && "all".equals(rule.getMatch_type()))
                    .forEach(rule -> rule.setMatch_value(""));
            cache.put("cacheRules", v.getCacheRules());
        }
    ...
    @Override
    public void saveAttackProtection(CdnDomain domain, SettingHigherVo setting) throws BusinessException {
        DomainAdvancedInfo.AttackProtection protection =
                setting == null ? null : setting.getAttackProtection();
        if (protection == null) {
            throw new BusinessException("攻击熔断配置不能为空");
        }
        int requestRate = protection.getRequestRate() == null ? 1000 : protection.getRequestRate();
        int burst = protection.getBurst() == null ? 2000 : protection.getBurst();
        int maxConnections = protection.getMaxConnections() == null ? 300 : protection.getMaxConnections();
        if (requestRate < 10 || requestRate > 100000) {
            throw new BusinessException("单域名请求速率必须在 10-100000 次/秒之间");
        }
        if (burst < 1 || burst > 1000000) {
            throw new BusinessException("单域名突发请求数必须在 1-1000000 之间");
        }
        if (maxConnections < 10 || maxConnections > 100000) {
            throw new BusinessException("单域名并发连接数必须在 10-100000 之间");
        }
        DomainAdvancedInfo.AttackProtection normalized = DomainAdvancedInfo.AttackProtection.builder()
                .status(normalizeSwitch(protection.getStatus()))
                .requestRate(requestRate)
                .burst(burst)
                .maxConnections(maxConnections)
                .build();
        saveAdvancedValue(domain, "attackProtection", normalized);
    }
    ...
    public DomainConfig getDomainConfig(CdnDomain domain) throws BusinessException {
        ...
        if ("configuring".equals(domainStatus)
        ...
        DomainAdvancedInfo.AttackProtection attackProtection =
                toBean(advancedJson.get("attackProtection"), DomainAdvancedInfo.AttackProtection.class);
        if (attackProtection == null) {
            attackProtection = DomainAdvancedInfo.AttackProtection.builder()
                    .status("on").requestRate(1000).burst(2000).maxConnections(300).build();
        } else {
            attackProtection.setStatus(normalizeSwitch(attackProtection.getStatus()));
            if (attackProtection.getRequestRate() == null) attackProtection.setRequestRate(1000);
            if (attackProtection.getBurst() == null) attackProtection.setBurst(2000);
            if (attackProtection.getMaxConnections() == null) attackProtection.setMaxConnections(300);
        }
